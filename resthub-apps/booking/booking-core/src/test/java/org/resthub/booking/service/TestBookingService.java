...
public class TestBookingService extends
        AbstractResourceServiceTest<Booking, BookingService> {
    ...
    public void testUpdate() {
        ...
        assertEquals("Card name should have been modified",
                CHANGED_TEST_CARD_NAME, booking.getCreditCardName());
    }

    @Test
    public void testFindByUser() {
        List<Booking> bookings = this.service
                .findByUserId(user.getId());
        assertEquals("bookings list should contain an unique result", 1,
                bookings.size());
        assertEquals("credit card names should be equals", TEST_CARD_NAME,
                bookings.get(0).getCreditCardName());
    }
}
