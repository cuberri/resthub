...
public class GroupServiceTest extends
        AbstractResourceServiceTest<Group, GenericResourceService<Group>> {
    ...
    @Named("groupService")
    @Override
    public void setService(GenericResourceService<Group> resourceService) {
    ...
    public void shouldDeleteGroupWithUsers() {
        ...
        assertFalse("The user shouldn't contain this group anymore", user
                .getGroups().contains(testGroup));
    ...
    public void shouldCreationBeNotified() {
        ...
        assertEquals(GroupServiceChange.GROUP_CREATION.name(),
                listener.lastType);
    ...
    public void shouldDeletionBeNotifiedById() {
        ...
        assertEquals(GroupServiceChange.GROUP_DELETION.name(),
                listener.lastType);
        assertArrayEquals(new Object[] { g }, listener.lastArguments);
    } 
    ...
    public void shouldGroupAdditionToGroupBeNotified() {
        ...
        ((GroupService) service).addGroupToGroup(g.getName(),
                subG.getName());

        // Then a deletion notification has been received
        assertEquals(GroupServiceChange.GROUP_ADDED_TO_GROUP.name(),
                listener.lastType);
    ...
    public void shouldGroupRemovalFromGroupBeNotified() {
        ...
        // When adding the user to the group
        ((GroupService) service).removeGroupFromGroup(g.getName(),
                subG.getName());

        // Then a deletion notification has been received
        assertEquals(GroupServiceChange.GROUP_REMOVED_FROM_GROUP.name(),
                listener.lastType);
        assertArrayEquals(new Object[] { subG, g }, listener.lastArguments);
