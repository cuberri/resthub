...
public class SpringComponentProviderFactory implements IoCComponentProviderFactory {
    ...
    private void registerSpringBeans(final ResourceConfig rc) {
        ...
        for (String name : names) {
        	...
        	Class<?> rawType = springContext.getType(name);
            Class<?> type = ClassUtils.getUserClass(rawType);
            if(type.getName().startsWith("$Proxy")) {
            	type = AopUtils.getTargetClass(springContext.getBean(name));
            }
    ...
    private static String getBeanName(ComponentContext cc, Class<?> c, ApplicationContext springContext) {
        ...
        names.addAll(Arrays.asList(BeanFactoryUtils.beanNamesForTypeIncludingAncestors(springContext, c)));
        

        if (names.size() == 0) {
        	if (ResourceConfig.isProviderClass(c) ||  ResourceConfig.isRootResourceClass(c)) {
        		for (Class<?> i : c.getInterfaces()) {
            		names.addAll(Arrays.asList(springContext.getBeanNamesForType(i)));
                }
        	} else {
        		return null;
        	}
        ...
        if (names.size() == 1) {
            ...
            for (String name : names) {
                ...
                if(beanType.getName().startsWith("$Proxy")) {
                	beanType = AopUtils.getTargetClass(springContext.getBean(name));
                }
